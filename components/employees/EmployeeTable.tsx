        ...
        <tbody>
          {employees.map(emp => (
            <tr key={emp.emp_id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 group">
              ...
              <td className="px-4 py-2.5 text-gray-500 dark:text-gray-400">{emp.branch || '-'}</td>
              ...
              <td className="px-4 py-2.5">
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(emp)} className="p-1 text-gray-400 dark:text-gray-500 hover:text-indigo-600"><Pencil size={14} /></button>
                  {onDelete && <button onClick={() => onDelete(emp)} className="p-1 text-gray-400 dark:text-gray-500 hover:text-red-500"><Trash2 size={14} /></button>}
                </div>
              </td>
            </tr>
          ))}
          {!employees.length && <tr><td colSpan={8} className="text-center py-10 text-gray-400 dark:text-gray-500">ไม่พบพนักงาน</td></tr>}
        </tbody>
      </table>
    </div>
  )
